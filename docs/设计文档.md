...
当前仓库尚未包含 Android 源码，因此本文档为初始设计方案，重点定义建议架构、模块边界、数据模型和主要流程。后续实现时可根据实际包名、UI 框架和接口协议调整。

## 2. 设计目标

- 支持工程式生图、对话式生图、图片编辑、图片变换和图片管理。
- 支持 OpenAI API，同时为第三方或自建 AI 服务预留适配层。
...
- 为业务层提供稳定数据接口。
- 屏蔽 OpenAI API、自定义 API、Room 和文件存储差异。
- 负责数据源选择、缓存和本地持久化。
